...
pRange.feedforwardWeight = 0.01:0.01:2.0;
pRange.intraInhibDirectWeight = -0.01:-0.02:-2.0;
pRange.theta = .05:.05:2;
...
em.nTimeBins = 500; %1,000 time bins
...
%                 simParam(simInd,4) = em.(fields{4});
                ...
                %run model w/ first value
                [frMatrix, ~, ~, ~] =  evidenceAccumulationModel(em);
                
                %calculate neurons per sec
                [~,ind] = max(frMatrix(1:em.nClusters,:));
                seqSpeed(simInd,1) = (ind(150)-ind(51))/5; %calc neurons per second
                
                %run model w/ second value
                em.globalExcAmp = 2*em.globalExcAmp;
                simParam(simInd,4) = em.globalExcAmp;
                [frMatrix, ~, ~, ~] =  evidenceAccumulationModel(em);
                
                %calculate neurons per sec
                [~,ind] = max(frMatrix(1:em.nClusters,:));
                seqSpeed(simInd,2) = (ind(150)-ind(51))/5; %calc neurons per second
